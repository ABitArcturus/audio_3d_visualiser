...
//import ThreeVis from "../visualiser/three-visualiser-three.js";
import ThreeVis from "../visualiser/three-visualiser-six.js";


//import load from "./visualiser/three-visualiser-two.js";

import { SelectedOscillatorStore, WaveformControl, FrequencyControl, AttackControl, DecayControl } from "./ui-controls.js";
...
// initializing the synthesizer keys
...
//3js
//const vis = new ThreeVis(analyser);
/////////////////////
/*
...
const detailFactorSlider = document.getElementById('detail-factor-slider');
const amplitudeHeightSlider = document.getElementById('amplitude-height-slider');
const detailFactorValue = document.getElementById('detail-factor-value');
const amplitudeHeightValue = document.getElementById('amplitude-height-value');

detailFactorSlider.addEventListener('input', (e) => {
    const factor = parseInt(e.target.value, 10);
    detailFactorValue.textContent = factor;
    vis.setDetailFactor(factor);
});

amplitudeHeightSlider.addEventListener('input', (e) => {
    const height = parseFloat(e.target.value);
    amplitudeHeightValue.textContent = height.toFixed(1);
    vis.setAmplitudeHeight(height);
}); */
/////////////////////
...
const freqencyControl = new FrequencyControl("frequency", "frequency-value", selectedOscillator);
const waveformControl = new WaveformControl("waveform", selectedOscillator);
...
window.addEventListener("click", (event) => {
    ...
    if (targetKey.getAttribute("synthesizer-key")) {
        ...
        if (currentlySelectedKey === targetKey) { // deselect
            ...
            // TODO brauche ich noch?
            selectedOscillatorIndex = targetKey.getAttribute("index");
